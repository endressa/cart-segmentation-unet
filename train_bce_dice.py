...
CHECKPOINT_PATH = Path("~/sarah/background_segmentation/checkpoints_smp/dice_model_1.pth").expanduser()
...
def train():
    ...
    metrics_csv = CHECKPOINT_PATH.parent / "training_metrics.csv"
    if not metrics_csv.exists():
        with open(metrics_csv, "w", newline="") as f:
            csv.writer(f).writerow(["epoch", "train_loss", "val_loss", "val_dice", "val_iou"])
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} | Train {avg_train:.4f} | Val {avg_val:.4f} | Dice {avg_dice:.4f} | IoU {avg_iou:.4f}")

        with open(metrics_csv, "a", newline="") as f:
            csv.writer(f).writerow([epoch+1, avg_train, avg_val, avg_dice, avg_iou])
