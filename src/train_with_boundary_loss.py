...
def signed_distance_map_np(mask: np.ndarray) -> np.ndarray:
    """
    mask: HxW in {0,1}; returns SDF (float32), negative inside, positive outside.
    """
...
@torch.no_grad()
def batch_sdf_from_masks(targets: torch.Tensor) -> torch.Tensor:
    """
    targets: (B,1,H,W) in {0,1}
    returns: (B,1,H,W) float32 SDF normalized by max(H,W)
    """
    B, _, H, W = targets.shape
    sdfs = []
    for b in range(B):
        m = (targets[b, 0] > 0.5).detach().cpu().numpy().astype(np.uint8)
        sdf = signed_distance_map_np(m)
        sdf = sdf / max(H, W)  # scale invariance
        sdfs.append(torch.from_numpy(sdf)[None, None, ...])
    return torch.cat(sdfs, 0).to(targets.device)
...
class SymmetricBoundaryLoss(nn.Module):
    def __init__(self, t: float = 0.5, w_gt: float = 1.0, w_pred: float = 0.5,
                 use_abs: bool = True, scale: float = 1.0):
        """
        t:      threshold for hard pred mask used to build pred SDF
        w_gt:   weight for GT-SDF term
        w_pred: weight for Pred-SDF term (detached)
        use_abs: use |.| for numerical robustness
        scale:  global scale
        """
        super().__init__()
        self.t = t
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        # GT SDF
        sdf_gt = batch_sdf_from_masks(targets)

        # Pred SDF (DETACHED, built from binarized probs)
        with torch.no_grad():
            pred_mask = (probs > self.t).float()
            sdf_pred  = batch_sdf_from_masks(pred_mask)

        # Two complementary terms:
        #   - encourage probs≈1 inside GT boundary (sdf_gt<0)
        #   - encourage probs≈1 inside its OWN predicted boundary (via (1-probs) * (-sdf_pred))
        term_gt   = probs * sdf_gt
        term_pred = (1.0 - probs) * (-sdf_pred)

        if self.use_abs:
            term_gt   = term_gt.abs()
            term_pred = term_pred.abs()

        loss = self.w_gt * term_gt.mean() + self.w_pred * term_pred.mean()
        return self.scale * loss


class CompositeSegLoss(nn.Module):
    """
    (BCE + Dice) combined with Symmetric Boundary Loss.
    """
    def __init__(self, bce_weight=0.5, boundary_weight=0.3,
                 sym_kwargs=None):
        super().__init__()
        self.region = BCEDiceLoss(bce_weight=bce_weight, smooth=1.0)
        sym_kwargs = sym_kwargs or {}
        self.boundary = SymmetricBoundaryLoss(**sym_kwargs)
        self.wb = boundary_weight

    def forward(self, logits, targets):
        reg = self.region(logits, targets)
        bnd = self.boundary(logits, targets)
        return (1 - self.wb) * reg + self.wb * bnd
...
def train():
    ...
    criterion = CompositeSegLoss(
        bce_weight=0.5,          # as before
        boundary_weight=0.3,     # try 0.2–0.4
        sym_kwargs=dict(t=0.5, w_gt=1.0, w_pred=0.5, use_abs=True, scale=1.0)
    )
    ...
    best_val_dice = -1.0
    epochs_no_improve = 0
    metrics_csv = CHECKPOINT_PATH.parent / "training_metrics_11.csv"
