...
CHECKPOINT_PATH = Path("~/sarah/background_segmentation/checkpoints_pretrained/pseudo_model_all.pth").expanduser()
CHECKPOINT_PATH.parent.mkdir(parents=True, exist_ok=True)
METRICS_CSV = CHECKPOINT_PATH.parent / "training_metrics_pseudo_sessions.csv"
...
def train():
    ...
    if not METRICS_CSV.exists():
        with open(METRICS_CSV, "w") as f:
            f.write("epoch,phase,train_loss,val_loss,val_dice\n")
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} | Phase {phase} | Train {avg_train:.4f} | Val {avg_val:.4f} | Dice {avg_dice:.4f}")

        with open(METRICS_CSV, "a") as f:
            f.write(f"{epoch+1},{phase},{avg_train:.6f},{avg_val:.6f},{avg_dice:.6f}\n")
