# this still has the enocder completely frozen
# maybe change training strategy: work with learning rates: very slow encoder, not as slow decoder, fast head
...
class FocalLoss(nn.Module):
    def __init__(self, alpha: float = 0.25, gamma: float = 2.0, reduction: str = "mean"):
        """
        alpha: Gewichtung der positiven Klasse (Cart). Typisch 0.25–0.75
        gamma: Fokus-Stärke (typisch 2.0)
        reduction: "mean" oder "sum"
        """
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
...
class FocalDiceLoss(nn.Module):
    def __init__(self, alpha=0.5, gamma=2.0, smooth=1.0, w_focal=0.5):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        return self.wf * focal + (1 - self.wf) * dice
...
def train():
    ...
    model = build_model()
    criterion = FocalDiceLoss(alpha=0.5, gamma=2.0, w_focal=0.7)

    # Encoder nicht einfrieren ===
    for p in model.encoder.parameters():
        p.requires_grad = True

    # Optimizer nur für Encoder + Decoder + Head
    optimizer = optim.AdamW([
        {"params": model.encoder.parameters(), "lr": LR * 0.1},   # z. B. 1e-5
        {"params": model.decoder.parameters(), "lr": LR},         # z. B. 1e-4
        {"params": model.segmentation_head.parameters(), "lr": LR * 3},  # z. B. 3e-4
    ...
    for epoch in range(EPOCHS):
        ...
        # current LRs
        lr_enc, lr_dec, lr_head = [pg["lr"] for pg in optimizer.param_groups]
        ...
        print(
            ...
            f"LRs E/D/H {lr_enc:.6f}/{lr_dec:.6f}/{lr_head:.6f}"
        )
