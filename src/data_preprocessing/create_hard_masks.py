...
# ====== CONFIG ======
IMAGE_ROOT = Path("/home/sarah/Documents/background_segmentation/whole_datasets/ariel_sarah/raw")
FIXED_DIR  = Path("/home/sarah/Documents/background_segmentation/dataset/hard_cases")
FIXED_DIR.mkdir(parents=True, exist_ok=True)
MODEL_PATH = Path("/home/sarah/Documents/background_segmentation/cart_segmentation/cart-segmentation-unet/pseudo_model_clean_13.pth")  # UNet / Dice model
SAM_PATH   = Path("/home/sarah/Documents/background_segmentation/cart_segmentation_cartClipper/src/models/sam_vit_b_01ec64.pth")

# ---- HARD CASES LIST (from the miner) ----
HARD_CSV = Path("/home/sarah/Documents/background_segmentation/whole_datasets/hard_mining_out/hard_candidates.csv")   # set to your csv path
TOPK = None            # how many to review (None = all)
MIN_COMPOSITE = None  # e.g. 0.12 to filter by score; None = no x
PRESERVE_ORDER = True # keep ranked order from CSV (recommended); False = shuffle
...
def load_hard_image_paths(csv_path: Path, topk=None, min_composite=None):
    if not csv_path.exists():
        print(f"⚠️ hard-cases CSV not found: {csv_path}")
        return []
    df = pd.read_csv(csv_path)
    if "path" not in df.columns:
        print("⚠️ CSV has no 'path' column.")
        return []
    if "composite" in df.columns and min_composite is not None:
        df = df[df["composite"] >= float(min_composite)]
    ...
    print(f"➡️ Loaded {len(paths)} hard candidates from CSV")
    return paths
...
# ====== Build image list (prefer hard-cases CSV) ======
paired_images = []
skipped_existing = 0

# Prefer the ranked list from the miner:
hard_list = load_hard_image_paths(HARD_CSV, topk=TOPK, min_composite=MIN_COMPOSITE)

if hard_list:
    candidate_imgs = hard_list
else:
    # Fallback: original recursive scan
    candidate_imgs = list(IMAGE_ROOT.rglob("*.jpeg"))
    print(f"⚠️ Using fallback scan. Found {len(candidate_imgs)} images under {IMAGE_ROOT}")

for img_path in candidate_imgs:
    rel = img_path.relative_to(IMAGE_ROOT) if IMAGE_ROOT in img_path.parents else Path(*img_path.parts[-3:])  # robust relpath
    out_path  = FIXED_DIR / rel.with_suffix(".png")
    skip_path = FIXED_DIR / rel.with_suffix(".skip")

    if out_path.exists() or skip_path.exists():
        skipped_existing += 1
        continue
    paired_images.append(img_path)

print(f"Found {len(paired_images)} images to refine. Skipped {skipped_existing} already done/marked.")
if not PRESERVE_ORDER:
    random.shuffle(paired_images)  # otherwise keep CSV ranking order
