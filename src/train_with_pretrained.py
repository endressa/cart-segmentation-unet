...
MASKS_ROOT  = Path("~/sarah/background_segmentation/2000_masks").expanduser()

# where to save the model + metrics
CHECKPOINT_PATH = Path("~/sarah/background_segmentation/checkpoints_pretrained/final_mask_model_5.pth").expanduser()
...
def train():
    ...
    best_val_dice = -1.0
    epochs_no_improve = 0
    metrics_csv = CHECKPOINT_PATH.parent / "training_metrics_5.csv"
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch == PHASE1_EPOCHS:
            print("🔓 Unfreeze Encoder + kleineres LR für Encoder")
            for p in model.encoder.parameters():
